...
function Product() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Kiểm tra nếu có ảnh tải lên, thực hiện upload ảnh trước khi tạo sản phẩm
    ...
    const productData = { ...form, avata: avataUrl };

    if (isEditing) {
      // Sửa sản phẩm
      axios.put(`${apiUrl}/${form.id}`, productData)
        .then(() => {
    ...
    <div>
      <div className='flex flex-row justify-center items-center py-4 border rounded-lg mb-4 px-4 bg-white'>
        ...
        <div className='basis-1/2 text-right'>
          <Link to="/admin/product/create" className="text-sm ml-2">Tạo mới</Link>
      ...
      <div className="bg-white p-3 border rounded-lg">
        <table className="table-auto w-full text-center border-collapse">
          ...
          <tbody className="divide-y">
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan="6" className="p-4 text-gray-500">Chưa có sản phẩm nào.</td>
              </tr>
            )}
          </tbody>
        </table>
